...
app.get('/api/admin/bots/lookup/:dnis', async (req, res) => {
  try {
    ...
    const response = await axios.get(`https://ui-api.torisedigital.com/api/admin/bots/lookup/${dnis}`);
...
app.post('/api/reports/entry', async (req, res) => {
  try {
    const response = await axios.post('https://ui-api.torisedigital.com/api/reports/entry', req.body);
    res.json(response.data);
  } catch (err) {
